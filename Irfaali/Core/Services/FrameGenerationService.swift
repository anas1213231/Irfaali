...
final class FrameGenerationService {
    ...
    private static func performGeneration(
        ...
        guard let videoTrack = tracks.first else { throw GenerationError.missingVideoTrack }

        let naturalSize = try await videoTrack.load(.naturalSize)
        let preferredTransform = try await videoTrack.load(.preferredTransform)
        let width = Int(abs(naturalSize.width).rounded())
        let height = Int(abs(naturalSize.height).rounded())
        guard width > 1, height > 1 else { throw GenerationError.invalidVideoDimensions }
        ...
        do {
            ...
            do { reader = try AVAssetReader(asset: sourceAsset) }
            catch { throw GenerationError.cannotCreateReader }
            activeReader = reader
            ...
            guard reader.canAdd(readerOutput) else { throw GenerationError.cannotAddReaderOutput }
            reader.add(readerOutput)

            let writer: AVAssetWriter
            do { writer = try AVAssetWriter(outputURL: silentURL, fileType: .mp4) }
            catch { throw GenerationError.cannotCreateWriter }
            activeWriter = writer

            // The intermediate optical-flow encode now uses the same quality policy
            // as final delivery. This prevents synthesized motion detail from being
            // discarded before the final export stage gets a chance to preserve it.
            let requestedBitrate = Int(VideoEncodingQualityPolicy.targetBitrate(
            ...
            guard writer.canAdd(writerInput) else { throw GenerationError.cannotAddWriterInput }
            ...
            guard writer.startWriting() else { throw GenerationError.writerFailed(writer.error?.localizedDescription ?? "startWriting failed") }
            ...
            while let sample = readerOutput.copyNextSampleBuffer() {
                ...
                guard let currentBuffer = CMSampleBufferGetImageBuffer(sample) else {
                    reader.cancelReading(); writer.cancelWriting(); throw GenerationError.missingImageBuffer
                }
                let currentPTS = CMSampleBufferGetPresentationTimeStamp(sample)
                if originPTS == nil { originPTS = currentPTS }
                ...
                if let previousBuffer, let previousPTS, let originPTS {
                    try autoreleasepool {
                        ...
                        for sample in samples {
                            ...
                            if sample.fraction < 0.000001 {
                                ...
                                let frame = try synthesizer.synthesize(source: previousBuffer, target: currentBuffer, forwardFlow: forward!, backwardFlow: backward!, fraction: sample.fraction)
                    ...
                    progress(min(max((max(0, CMTimeGetSeconds(adjustedCurrent)) / durationSeconds) * 0.88, 0), 0.88))
                }
            ...
            if let previousBuffer, let previousPTS, let originPTS {
                ...
                for sample in plan.samples(from: start, to: end, nextIndex: &nextOutputIndex) {
                    try append(previousBuffer, at: CMTime(seconds: sample.time, preferredTimescale: 60_000), adaptor: adaptor, writerInput: writerInput, writer: writer)
                    if sample.fraction < 0.000001 { sourceFrameCount += 1 }
                    else { sceneCutFallbackFrameCount += 1 }
                }
            }
            ...
            activeWriter = nil; activeReader = nil; progress(0.9)

            try Task.checkCancellation()
            let muxedURL = try await muxOriginalAudio(generatedVideoURL: silentURL, originalURL: sourceURL, destinationURL: finalURL)
            try? FileManager.default.removeItem(at: silentURL)
            try Task.checkCancellation(); progress(1)
            return Result(url: muxedURL, generatedFrameCount: generatedFrameCount, sceneCutFallbackFrameCount: sceneCutFallbackFrameCount, sourceFrameCount: sourceFrameCount, targetFPS: plan.targetFPS)
        } catch {
            activeReader?.cancelReading(); activeWriter?.cancelWriting()
            try? FileManager.default.removeItem(at: silentURL); try? FileManager.default.removeItem(at: finalURL)
            throw error
        }
    }

    private static func append(_ pixelBuffer: CVPixelBuffer, at presentationTime: CMTime, adaptor: AVAssetWriterInputPixelBufferAdaptor, writerInput: AVAssetWriterInput, writer: AVAssetWriter) throws {
        try Task.checkCancellation()
        let deadline = Date().addingTimeInterval(30)
        while !writerInput.isReadyForMoreMediaData {
            guard Date() < deadline else { throw GenerationError.writerFailed("The encoder stopped accepting frames for 30 seconds.") }
            try Task.checkCancellation()
            if writer.status == .failed { throw GenerationError.writerFailed(writer.error?.localizedDescription ?? "Writer became unavailable") }
            if writer.status == .cancelled { throw CancellationError() }
            Thread.sleep(forTimeInterval: 0.0015)
        }
        guard adaptor.append(pixelBuffer, withPresentationTime: presentationTime) else {
            if let error = writer.error { throw GenerationError.writerFailed(error.localizedDescription) }
    ...
    private static func finish(_ writer: AVAssetWriter) async throws {
        ...
        }, onCancel: { writer.cancelWriting() })
    }

    private static func muxOriginalAudio(generatedVideoURL: URL, originalURL: URL, destinationURL: URL) async throws -> URL {
        try Task.checkCancellation()
        ...
        guard let generatedTrack = generatedTracks.first else { throw GenerationError.missingVideoTrack }
        let audioTracks = try await originalAsset.loadTracks(withMediaType: .audio)
        if audioTracks.isEmpty {
            try Task.checkCancellation(); try FileManager.default.moveItem(at: generatedVideoURL, to: destinationURL); try Task.checkCancellation(); return destinationURL
        }
        ...
        guard let videoCompositionTrack = composition.addMutableTrack(withMediaType: .video, preferredTrackID: kCMPersistentTrackID_Invalid) else { throw GenerationError.cannotMuxAudio }
        try videoCompositionTrack.insertTimeRange(CMTimeRange(start: .zero, duration: duration), of: generatedTrack, at: .zero)
        videoCompositionTrack.preferredTransform = try await generatedTrack.load(.preferredTransform)
        for audioTrack in audioTracks {
            try Task.checkCancellation()
            guard let compositionAudioTrack = composition.addMutableTrack(withMediaType: .audio, preferredTrackID: kCMPersistentTrackID_Invalid) else { throw GenerationError.cannotMuxAudio }
            try compositionAudioTrack.insertTimeRange(CMTimeRange(start: .zero, duration: duration), of: audioTrack, at: .zero)
        }
        try? FileManager.default.removeItem(at: destinationURL)
        guard let export = AVAssetExportSession(asset: composition, presetName: AVAssetExportPresetPassthrough) else { throw GenerationError.cannotMuxAudio }
        try await withTaskCancellationHandler(operation: {
            do { try await export.export(to: destinationURL, as: .mp4) }
            catch { if Task.isCancelled { throw CancellationError() }; throw GenerationError.cannotMuxAudio }
    ...
    private static func makeDestination(prefix: String) throws -> URL {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent("Irfaali", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        return directory.appendingPathComponent("\(prefix)-\(UUID().uuidString).mp4")
    }
}